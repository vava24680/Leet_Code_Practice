...
class Solution:
    def getIntersectionNode(
        ...
        #
        # If two singly linked list has a intersection node, the length after
        # the intersection node of a list is as same as the other one.
        #
        ...
        #
        # The reason why we need the pseudo node NULL is because if we just
        # check if current node is the tail node of list a or list b and change
        # it the other list's head, we will not be able to break the loop
        # if there is no intersection node in the given two lists because
        # it's impossible that list a current node and list b current node are
        # the same if they have different length.
        #
        ...
        while list_ab_node is not list_ba_node:
            if not list_ab_node:
                list_ab_node = headB
            else:
                list_ab_node = (list_ab_node.next)

            if not list_ba_node:
                list_ba_node = headA
            else:
                list_ba_node = (list_ba_node.next)
