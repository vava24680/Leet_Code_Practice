...
def isObstacle(
    current_coordinate_tuple: Tuple[int, int],
    obstacles_coordinates_tuple_list: List[Tuple[int, int]]
) -> bool:
    first_larger_or_equal_index = bisect_left(
        obstacles_coordinates_tuple_list,
        current_coordinate_tuple
    )

    if (
        (
            len(obstacles_coordinates_tuple_list)
            ==
            first_larger_or_equal_index
        )
        or
        (
            current_coordinate_tuple
...
class Solution:
    def robotSim(self, commands: List[int], obstacles: List[List[int]]) -> int:
        ...
        obstacles_coordinates_tuple_list = [(x, y) for x, y in obstacles]
        current_x_coordinate = 0
        current_y_coordinate = 0

        # Sort the obstacle coordinates list in the ascending order.
        obstacles_coordinates_tuple_list.sort()
