...
def isObstacle(
    current_coordinate_list: List[int],
    obstacles_coordinates_list: List[List[int]]
) -> bool:
    first_larger_or_equal_index = bisect_left(
        obstacles_coordinates_list,
        current_coordinate_list
    )

    if (
        (len(obstacles_coordinates_list) == first_larger_or_equal_index)
        or
        (
            current_coordinate_list
            !=
            (obstacles_coordinates_list[first_larger_or_equal_index])
        )
    ):
        return False
